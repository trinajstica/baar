...
/* Helper: format size for human-readable output */
...
/* Check if file is supported by libarchive */
...
const char *la_get_format(const char *archive_path) {
    ...
    /* Read first header to detect format */
    ...
    if (format && format[0]) {
        ...
        /* Fallback: detect by extension if archive_format_name returns NULL */
...
/* Extract single file to absolute path (for drag & drop, no chdir) */
int la_extract_to_path(const char *archive_path, const char *entry_name, 
    ...
    while ((rcode = archive_read_next_header(a, &entry)) == ARCHIVE_OK) {
        ...
        if (strcmp(name, entry_name) == 0) {
            ...
            /* Set the absolute destination path */
...
int la_test(const char *archive_path, const char *password) {
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        /* Read all data to verify decompression */
        const void *buff;
        size_t size;
        int64_t offset;
...
/* Add files to archive (creates new or recreates with additions) */
int la_add_files(const char *archive_path, const char **file_paths, 
    ...
    if (stat(archive_path, &st) == 0 && st.st_size > 0) {
        /* Check if it's a valid archive (not just empty file) */
    ...
    /* Uporabi sistemski /tmp za začasne datoteke */
    ...
    /* Detect format from extension and set compression */
    const char *ext = strrchr(archive_path, '.');
    
    /* Normalize compression level: 0=store, 1-2 -> 6 (good default), 3-9 as-is */
    int level = compression_level;
    if (level < 0 || level > 9) level = 6;  // default
    if (level >= 1 && level <= 2) level = 6; // ensure decent compression

    /* If this is a ZIP and a password is provided, prefer calling the system 'zip'
       tool to add/update entries because libarchive's zip write/encryption support
       can be unreliable across versions. Using the native 'zip' avoids writer-side
       encryption failures. */
    if (ext && strcmp(ext, ".zip") == 0 && password && password[0]) {
        /* Build argv: zip -P <password> -j <archive_path> <file1> <file2> ... */
        char **argv = malloc(sizeof(char*) * (file_count + 7));
        int ai = 0;
        argv[ai++] = "zip";
        argv[ai++] = "-P";
        argv[ai++] = (char*)password;
        argv[ai++] = "-j"; /* junk paths to store basenames */
    ...
    if (ext && strcmp(ext, ".zip") == 0) {
        ...
        /* Build options string including compression level and optional encryption/passphrase */
        char opt[512];
        ...
        char opt[64];
        int bz2_level = (level == 0) ? 1 : ((level > 6) ? 9 : level); // bzip2: 1-9, default 9 for best
     ...
     /* Encryption for other formats (7z/tar.*) is not configured here because
         libarchive cannot write encrypted 7z or tar with passphrase protection.
         We only handle ZIP as above when a password is provided. */
    ...
    /* If archive exists, copy existing entries first */
    if (archive_exists) {
        ...
        /* Add password if provided - needed to read encrypted entries */
        if (password) {
        ...
        if (archive_read_open_filename(in, archive_path, 10240) == ARCHIVE_OK) {
            ...
            while (archive_read_next_header(in, &entry) == ARCHIVE_OK) {
                ...
                /* Read and copy data blocks; if any read error occurs, abort the whole operation
                   to avoid creating entries with zero-length bodies (which corrupts the archive). */
                int copy_failed = 0;
                ...
                if (rdcode != ARCHIVE_EOF || copy_failed) {
                    ...
                    archive_write_free(out);
                    /* remove temporary file to avoid leaving corrupted archive */
                    unlink(temp_path);
                    return 1;
                }
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        /* Use basename for entry name instead of full path */
    ...
    /* Replace original with temp file, robustno tudi med filesistemi */
    if (rename(temp_path, archive_path) != 0) {
        if (errno == EXDEV) {
            /* Cross-device: kopiraj ročno */
